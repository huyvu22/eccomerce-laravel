                ...
                @foreach(Cart::content() as $item)
                    <li class="mini_cart_{{$item->rowId}}">
                        <div class="wsus__cart_img">
                            <a href="{{route('product-detail', $item->options->slug)}}"><img src="{{asset($item->options->image)}}" alt="product" class="img-fluid w-100"></a>
                            <form class="form-delete-item" action="{{ route('cart.remove-sidebar-product') }}" method="post">
                                @csrf
