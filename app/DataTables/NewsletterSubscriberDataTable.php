...
class NewsletterSubscriberDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($query){
                return '
                    <form class="form-delete me-2" style="display:inline-block" action="'.route('admin.subscriber.destroy', $query).'" method="POST">
                        ' . csrf_field() . '
                        ' . method_field('DELETE') . '
            ...
            ->addColumn('is_verified',function ($query){
                return $query->is_verified ==1 ?'<i class="badge badge-success">Yes</i>' :'<i class="badge badge-danger">No</i>';
            })
     ...
     */
    public function query(NewsletterSubscrible $model): QueryBuilder
    {
