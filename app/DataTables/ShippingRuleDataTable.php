        ...
        return (new EloquentDataTable($query))
            ->addColumn('status', function ($query) {
                ...
                return '<label class="custom-switch switch-status mt-2" style="cursor: pointer">
                            <form class="form-status" action="'.route('admin.shipping-rule.update', $query).'" method="post" type="submit">
                                    ' . csrf_field() . '
                                    ' . method_field('PUT') . '
            ...
            ->addColumn('action', function ($query){
                $editBtn= '<a href="'.route('admin.shipping-rule.edit', $query).'" class="btn btn-primary"><i class="fas fa-edit"></i></a>';
                $deleteBtn= ' <form class="form-delete" style="display:inline-block" action="'.route('admin.shipping-rule.destroy', $query).'" method="POST">
                                ' . csrf_field() . '
                                ' . method_field('DELETE') . '
            ...
            ->addColumn('type', function ($query){
                if($query->type == 'flat_cost'){
                    return '<span class="badge badge-secondary">Flat Amount</span>';
                }else{
                    return '<span class="badge badge-success">Min Order Amount</span>';
                }
            })
            ->addColumn('cost', function ($query){
              return format($query->cost);
            })
            ->addColumn('min_cost', function ($query){
                return format($query->min_cost);
            })
