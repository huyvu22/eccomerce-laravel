...
class ProductImageGalleryDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('image',function ($query){
                return '<img width="100" height="cover" src="'.asset($query->image).'">';
            })
            ->addColumn('action', function ($query){
                return '
                        <form class="form-delete me-2" style="display:inline-block" action="'.route('admin.products-image-gallery.destroy', $query).'" method="POST">
                            ' . csrf_field() . '
                            ' . method_field('DELETE') . '
    ...
    {
        return $this->builder()
                    ->setTableId('product-imagegallery-table')
