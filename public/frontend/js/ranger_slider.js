...
(function ($) {
	...
	FlatSliderObj = {
		...
		_create: function () {
			...
			if (this.options.range === true) {
				this.$slider_container.append(
					'<div class="min_value">' + formatNumber(this.options.values[0]) + ' ' + this.options.einheit + '</div>'
					+ '<div class="max_value">' + formatNumber(this.options.values[1]) + ' ' + this.options.einheit + '</div>');
			} else {
				this.$slider_container.append(
		...
		on_slide: function (event, ui) {
			if (this.options.range === true) {
				this.element.val(formatNumber(ui.values[0]) + ';' + formatNumber(ui.values[1]));
				this.$min_value.html(formatNumber(ui.values[0]) + ' ' + this.options.einheit);
				this.$max_value.html(formatNumber(ui.values[1]) + ' ' + this.options.einheit);
				this._update_range_handle($(ui.handle));
			} else {
				this.element.val(formatNumber(ui.value) + ' ' + this.options.einheit);
				this.$wert.html(formatNumber(ui.value) + ' ' + this.options.einheit);
